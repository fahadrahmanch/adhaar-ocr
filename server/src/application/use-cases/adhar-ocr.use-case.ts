...
import { AadhaarValidationService } from "../services/AadhaarValidationService";
import Tesseract from "tesseract.js";
import sharp from "sharp";

export class AdharOcrUseCase implements IAdharOcrUseCase {
  constructor(
  ...
  async execute(
    ...
    if (!frontImage || !backImage) {
      throw new Error(
        "Front and back Aadhaar images are required"
      );
    }

    // preprocess images
    await sharp(frontImage.path)
      .grayscale()
      .normalize()
      .sharpen()
      .toFile("uploads/front-processed.jpg");

    await sharp(backImage.path)
      .grayscale()
      .normalize()
      .sharpen()
    ...
    const frontResult = await Tesseract.recognize(
      "uploads/front-processed.jpg",
      "eng+mal"
    );

    const backResult = await Tesseract.recognize(
      "uploads/back-processed.jpg",
      "eng+mal"
    );
...
console.log("actual back text :",actualBackText);
    // validation
    const isValidFront =
      this.aadhaarValidationService.isAadhaarFront(
        actualFrontText
      );
    ...
    const isValidBack =
      this.aadhaarValidationService.isAadhaarBack(
        actualBackText
      );
